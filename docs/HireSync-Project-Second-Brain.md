...
**Status: v1.1 FROZEN CANONICAL BASELINE — approved for implementation**
...
| Last audit state | Independent v1.1 freeze audit completed 29 August 2026; Section 53 verdict `READY TO FREEZE`; canonical baseline approved for implementation |
...
1. Original Smart Recruitment Matching Platform BRD.
2. Approved HireSync technical baseline.
3. The 12 approved business and matching rules dated 29 August 2026.
4. Other previously approved HireSync business decisions.
5. Safe, clearly labelled derived engineering decisions in this document.
6. The v1.0 Second Brain as historical implementation context.
7. The forensic audit as a defect checklist, not a requirements authority.
8. The reference Second Brain image as a structure and quality guide only.
...
| BRD-OS-02 | Email/SMS notifications | No sending provider, template, queue, or notification job |
...
| GAP-11 | Registration verification, password reset, account deletion, and email verification are absent | **[D - Future]** excluded from baseline and listed as future enhancements. |
...
|---|---|---|
| Secure role-based access | [A - BRD][B - Approved] | Anonymous/incorrect-role requests are rejected; passwords are Identity-hashed |
...
|---|---|---|
| Identity and Access | Register Job Seekers/Employers, login/logout, JWT, roles, suspension | [A - BRD][B - Approved][C - Derived] |
...
| Administration | User status management and three required usage counts | [A - BRD][B - Approved] |
...
- Password reset, email verification, social login, multi-factor authentication, account deletion, saved jobs, employer billing, advanced analytics, audit-history UI, multilingual UI, geospatial distance, skill synonyms/ontology, and CV version history. **[D - Future]**
...
2. Create an Open vacancy with structured requirements.
3. Update it while Open or close it.
4. Open applicant list -> review deterministic descending ranking and authorized structured candidate data; no CV is exposed.
5. Change application status -> Job Seeker notification is created.
6. Send one contact request -> observe Pending/Accepted/Declined status only.

### 15.3 Administrator journey

1. Sign in using a securely seeded account.
2. View totals for users, vacancies, and applications.
3. Search accounts -> suspend/reactivate an eligible Job Seeker/Employer.
4. Never access a match-edit operation because none exists.
...
| Main flow | Enter display name, email, password, confirmation, role -> client validates -> API normalizes email -> Identity creates hashed credential -> assigns exactly one selected role -> creates empty role profile -> account is Active -> return 201 and direct user to login |
| Alternate flow | If already authenticated, redirect to role home; changing role before submit changes onboarding destination |
| Validation | Display name 2-100 chars; valid email <=256; password 8-128 with upper, lower, digit, and non-alphanumeric; confirmation exact; role allow-list contains only JobSeeker/Employer |
| Failure/error paths | Duplicate email 409; invalid fields 400; role injection 400; rate limit 429; persistence failure 500 with no partial account/profile |
| Postconditions | Identity user, role membership, and matching empty profile exist atomically; no JWT is issued by registration |
| Journey/UI | `/auth/register`; clear role cards, form, password rules, disabled submitting state, success message then login link/redirect |
| Screen responsibility | Never expose Administrator option; retain non-password fields after validation error; announce errors accessibly |
| Business logic | Email is case-insensitive unique; account defaults Active; Administrator can only be seeded |
| NFR | Password never logged/returned; response target <=2 seconds on demo data; keyboard-accessible form |
| API | `POST /api/v1/auth/register` |
| Data affected | `AspNetUsers`, `AspNetRoles/UserRoles`, `JobSeekerProfiles` or `EmployerProfiles` |
| Authorization | Anonymous only; endpoint rate-limited |
| Acceptance criteria | Valid role account can subsequently login; invalid/duplicate data creates no extra records |
| Test scenarios | One valid case per role; duplicate case-insensitive email; weak password; forged Administrator role; transactional rollback |
| Provenance | [A - BRD] registration; [B - Approved] Identity/roles; [C - Derived] fields, policy, atomic profile creation |
...
| Preconditions | Existing Active account and correct credentials |
| Trigger | Submit Login form |
| Main flow | Validate input -> Identity verifies hashed password -> check Active status -> issue 30-minute JWT with subject, email, role, and token-version claims -> return minimal user/session DTO -> route to role dashboard |
...
| Failure/error paths | Invalid credentials 401; suspended 403 with `ACCOUNT_SUSPENDED`; rate limit 429; server error 500 |
...
| Business logic | Account role and TokenVersion come from server data; no refresh token in baseline |
...
| Test scenarios | Each role; wrong password; unknown email same outward error; suspended user; expired/tampered token; rate limit |
...
| Preconditions | Authenticated Active Employer |
...
| Data affected | `EmployerProfiles` |
...
| Preconditions | Active Employer owns vacancy; status Open |
...
| Preconditions | Active Employer owns an Open vacancy |
...
| Main flow | API restricts to Open vacancies owned by Active Employers -> applies keyword and location filters -> calculates match values when profile is match-ready -> applies requested deterministic ordering -> returns bounded page and total count |
...
| Acceptance criteria | Only searchable Open/Active-employer vacancies appear; filters and pagination are stable; no dead-end on zero results |
| Test scenarios | No filters; keyword/location; combined; invalid sizes; inactive Employer; closed vacancy; incomplete profile; deterministic sort |
...
| Preconditions | Active Job Seeker; complete matching profile; current valid CV; Open vacancy owned by Active Employer; no prior application |
...
| Preconditions | Active Employer owns vacancy; application records may exist |
...
| Preconditions | Active Employer owns vacancy containing application; application not terminal |
...
| Preconditions | Active Employer owns the application's vacancy; candidate account exists; application is not Rejected; no request exists |
...
| Postconditions | Suspended target cannot login/use API; existing data remains; Active Employer's Open vacancies are hidden while suspended and reappear after reactivation unless Closed |
...
Identity permits role membership technically; HireSync registration/seed services enforce exactly one of `JobSeeker`, `Employer`, or `Administrator`. Startup/seed validation fails clearly on invalid role/profile combinations: JobSeeker has exactly one JobSeekerProfile, Employer has exactly one EmployerProfile, and Administrator has neither. Database triggers are not required.
...
| `CompanyName` | `nvarchar(150)` | Yes until complete | 2-150 when saved complete |
| `NormalizedCompanyName` | `nvarchar(150)` | Yes until complete | Search form |
| `Description` | `nvarchar(2000)` | Yes until complete | 20-2,000 |
| `Location` | `nvarchar(100)` | Yes until complete | Display value |
| `NormalizedLocation` | `nvarchar(100)` | Yes until complete | Search form |
| `CreatedAtUtc` / `UpdatedAtUtc` | `datetime2(3)` | No | UTC |
...
| `EmployerProfiles` | unique `UserId`; `NormalizedCompanyName` | One profile/user; job keyword query |
...
- Profile-to-user, vacancy-to-employer, application-to-vacancy/profile, CV-to-profile, contact-to-application, notification-to-recipient, and notification-to-application use `Restrict/NoAction` for historical safety.
...
|---|---|
| Registration | User + one role membership + role profile |
...
|---:|---|---|
| 400 | Invalid syntax, field value, enum, or transition | `VALIDATION_FAILED`, `INVALID_STATUS_TRANSITION`, `PROFILE_INCOMPLETE` |
| 401 | Missing, expired, invalid, or revoked JWT / invalid login | `AUTH_REQUIRED`, `TOKEN_INVALID`, `INVALID_CREDENTIALS` |
| 403 | Authenticated but role/account policy denies | `FORBIDDEN`, `ACCOUNT_SUSPENDED` |
| 404 | Resource absent or not owned by caller | `RESOURCE_NOT_FOUND` |
| 409 | Current state prevents operation/race/concurrency | `DUPLICATE_APPLICATION`, `VACANCY_CLOSED`, `CONCURRENCY_CONFLICT`, `CONTACT_ALREADY_EXISTS` |
...
|---|---|---|---|---|
| `POST /auth/register` | Anonymous | displayName, email, password, confirmPassword, role; rules in UC-AUTH-01; role only JobSeeker/Employer | 201 account summary without JWT | 400, 409 email, 429 |
| `POST /auth/login` | Anonymous | email/password required | 200 accessToken, expiresAtUtc, user `{id, displayName, email, role}` | 401 generic, 403 suspended, 429 |
...
| Method and route | Role | Request and validation | Success | Important errors |
|---|---|---|---|---|
| `GET /employer/vacancies` | Employer | optional status, pagination | 200 own paged vacancies | 400 |
| `POST /employer/vacancies` | Employer | title, description, location, minimumExperienceMonths, nullable requiredEducationLevel, skillNames | 201 Open vacancy DTO/rowVersion | 400 profile incomplete |
| `GET /employer/vacancies/{vacancyId}` | Employer | GUID and ownership | 200 owned detail/rowVersion | 404 non-owned |
| `PUT /employer/vacancies/{vacancyId}` | Employer | Full editable fields + rowVersion; must be Open | 200 updated DTO/new rowVersion | 400, 404, 409 closed/stale |
| `PATCH /employer/vacancies/{vacancyId}/status` | Employer | `{status:"Closed", rowVersion}` only | 200 closed DTO/new rowVersion | 400, 404, 409 |
| `GET /employer/vacancies/{vacancyId}/applicants` | Employer | optional status, pagination | 200 ranked page with breakdown/gaps/contact state | 400, 404 |
| `PATCH /employer/applications/{applicationId}/status` | Employer | target application status + rowVersion | 200 updated application; notification committed | 400 transition, 404, 409 stale/terminal |
...
There is intentionally no Administrator endpoint for match results/weights, CV access, role change, password inspection, vacancy editing, application editing, notifications, or unspecified settings. There is no Employer CV endpoint anywhere in the baseline.
...
sequenceDiagram
    ...
    I->>D: Read user, role, status, token version
    D-->>I: Account state
    I-->>A: Valid Active principal
    A-->>B: Signed 30-minute JWT + safe user DTO
    B->>A: Bearer JWT on protected request
    A->>D: Validate current token version/status
    A-->>B: Authorized resource or Problem Details
```
...
- No email confirmation or password reset in baseline; do not expose dead links.
...
Authorization has three independent gates:

1. **Authentication/account gate:** missing/invalid/expired/revoked JWT is 401. After cryptographic identity validation, a Suspended account is 403 `ACCOUNT_SUSPENDED`; for an Active account, token-version mismatch is revoked and returns 401.
2. **Role gate:** endpoint policy requires JobSeeker, Employer, or Administrator.
3. **Resource gate:** query includes current user/profile/employer/request recipient and returns 404 for absent or foreign records.
...
- Contact creation requires owning Active Employer, Active target, non-Rejected application, and no existing request; response returns status/context only and creates no notification.
...
The API fails fast with a clear local console error when the connection string, valid JWT signing key/issuer/audience, writable CV root/staging area, exact 5,000,000-byte limit, or required Administrator seed configuration is missing/invalid. In the localhost baseline, startup validates every HireSync account's exactly-one-role/profile invariant after idempotent role/Admin seeding and fails with a safe diagnostic if any account is malformed. It must not silently use insecure defaults.
...
- Profile/vacancy/registration/search validation boundaries.
...
- Registration/login/logout, Identity hashing indirectly verified (stored value not plaintext), JWT expiry/tamper/token-version/suspension.
...
|---|---|---|
| E2E-01 | Employer registers -> logs in -> completes company -> creates/updates Open vacancy | Role route, validation, persisted fields, Open state, responsive primary path |
...
|---|---|
| AUTH | Register only JobSeeker/Employer; atomic matching profile creation; duplicate identity; password policy; generic login failure; lockout/rate controls; expiry/tamper/revocation; 401 invalid token; 403 suspended valid account; exact role/profile seed invariant |
...
| APP | First apply; sequential and concurrent duplicate; Closed vacancy; inactive Employer; incomplete profile; missing CV; role/ownership; every allowed/disallowed Rule-10 transition; terminal states; same-state no-op; one notification for true change; none for no-op; rollback atomicity |
...
| ADM | `totalUsers` includes every role/status; optional breakdown reconciliation; role denial; search/filter; suspend/reactivate; self/Administrator protection; token-version invalidation; no match mutation/settings/CV route |
...
4. All five critical Playwright flows.
...
|---|---|
| BRD-SC-01 Registration/login/logout | FR-01 plus UC-AUTH-03 and `POST /auth/logout` |
| BRD-SC-02 JWT-secured access | Security Section 30 and authorization matrix/tests |
| BRD-SC-03 Job Seeker profile/CV | FR-02/03 |
| BRD-SC-04 Employer company profile | UC-EMP-01; GET/PUT employer profile; EmployerProfiles |
...
| BRD-SC-14 Admin accounts/dashboard | FR-12; unspecified basic settings remain OD-01 with safe baseline of no module |
...
| BRD-OS-02 Email/SMS notification | No provider/key/job/template/send method and no contact-data disclosure behavior | Config/dependency/DTO inventory; E2E observes in-app application-status notice only |
...
2. Employer: login, company profile, create/show structured vacancy.
...
- **Administrator:** "The Administrator manages account status and usage counts but cannot edit matching results."
...
10. No withdrawal, vacancy reopen/delete, account delete, password reset, email verification, or multi-factor flow.
...
- Email verification, secure password reset, multi-factor authentication, and refresh-token rotation.
...
| 5 | Every role permission | PASS - blueprint | Exactly one role/profile invariant, permission matrix, endpoint policies, ownership concealment, seeded Admin rule |
...
| 8 | Every status lifecycle | PASS - blueprint | Account Active/Suspended; Vacancy Open-to-Closed terminal; complete approved application graph; one terminal contact graph and cross-state rules |
...
| 22 | Administrator restrictions | PASS - blueprint | Accounts/counts only, protected Admin/self, no CV/business mutation/match/settings endpoint; OD-01 safe default |
...
| 25 | Authentication status codes | PASS - blueprint | 401 invalid/expired/revoked, 403 `ACCOUNT_SUSPENDED`, 403 `FORBIDDEN`, 404 concealed ownership across pipeline/UI/Swagger/tests |
...
| 27 | Testing obligations | PASS - blueprint | Exact matching/application/status/contact/CV/Admin/auth cases, SQL constraints, fault injection, Vitest and focused Playwright; no fabricated results |
...
| 30 | Demo/viva consistency | PASS - blueprint | Current-data, structured-only matching, owner-only CV, status-only contact, status-only notification event, Admin limits and evidence caveat use canonical wording |

### Critical Issues

None found in the v1.1 candidate.
...
- BRD-first provenance; all 12 newly approved rules are `[B - Approved]`.
...
- Identity/JWT/role/ownership protection with exact suspension semantics and no Administrator match override.
...
No matching, lifecycle, CV-access, contact-disclosure, notification, version-policy, or architecture behavior remains open.

### BRD Traceability Coverage %

**100% at blueprint level:** 12 of 12 functional requirements and 5 of 5 non-functional requirements have complete horizontal trace rows. Additionally, 14 of 14 in-scope statements and 7 of 7 explicit out-of-scope statements are cross-checked. Implementation evidence remains 0% until code/tests exist and is intentionally not conflated with documentation coverage.
...
| Employer | Registered role maintaining company/vacancies and applicant workflow |
...
This v1.1 frozen canonical baseline deliberately contains no application source code. OD-01 is non-blocking under its safe default; OD-02 is an operational repository-initialization item that must be filled before submission. Human developers and AI coding assistants must implement only linked issues, preserve all source labels/boundaries, run the specified tests, and update traceability through reviewed PRs.
...
**End of HireSync Project Second Brain - v1.1 FROZEN CANONICAL BASELINE**
